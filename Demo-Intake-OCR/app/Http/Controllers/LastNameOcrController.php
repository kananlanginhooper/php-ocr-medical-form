...
class LastNameOcrController extends Controller
{
    ...
    public function lastNameOcr()
    {
        try {
            ...
            return view('lastname-ocr', [
                ...
                'humanValue' => null,
                'statusType' => 'error',
                'statusMessage' => 'Unable to load last name OCR state. ' . $exception->getMessage(),
            ]);
        }
    }
    ...
    public function findLastNameLabel()
    {
        try {
            ...
            if (!$labelResult) {
                return response()->json([
                    'ok' => false,
                    'message' => 'Could not find label "Last Name" in image.',
                ], 400);
            }
            ...
            $state['fp_lastname_location'] = $labelLocationJson;
            $state['fp_lastname_ocr_score'] = $labelResult['confidence'] ?? null;
            ...
            \Log::warning('Error in findLastNameLabel: ' . $e->getMessage());
            return response()->json([
                'ok' => false,
    ...
    /**
     * Step 2: find underline location after Last Name label
     */
    ...
    public function findLastNameArea()
    {
        try {
            ...
            if (!$handwrittenResult) {
                return response()->json([
                    'ok' => false,
                    'message' => 'Could not find handwritten last-name area after label.',
                ], 400);
            }
            ...
            $currentLocationData['handwritten'] = [
                ...
                'source' => $handwrittenResult['source'] ?? 'last-last-label-gap',
            ...
            return response()->json([
                'ok' => true,
                'message' => 'Handwritten last-name area found successfully.',
                'handwritten' => $handwrittenResult,
            ]);
        } catch (Throwable $e) {
            \Log::warning('Error in findLastNameArea: ' . $e->getMessage());
            return response()->json([
                'ok' => false,
                'message' => 'Error: ' . $e->getMessage(),
            ], 500);
    ...
    public function findLastNameOptions(Request $request)
    {
        try {
            ...
            $state = array_merge($state, [
                'gs_current_image_path' => $pendingData['fp_image_path'] ?? null,
                'fp_lastname_ocr' => $top['text'],
                'fp_lastname_ocr_score' => $top['confidence'],
                'fp_lastname_ocr_options' => json_encode($options),
                'updated_at' => now()->toDateTimeString(),
            ]);
            session(['global_state' => $state]);
            ...
            \Log::warning('Error in findLastNameOptions: ' . $e->getMessage());
            return response()->json([
                'ok' => false,
                'message' => 'Error: ' . $e->getMessage(),
            ], 500);
    ...
    public function runLastNameOcr(Request $request)
    {
        ...
        if ($selectedRecordId <= 0) {
            return redirect()->route('lastname.index')->with([
                'statusType' => 'error',
        ...
        if (is_array($globalState) && !empty($globalState['fp_lastname_location'])) {
            $existingLocationData = json_decode($globalState['fp_lastname_location'], true) ?: [];
        }
    ...
    public function confirmLastNameOcr(Request $request)
    {
        $validated = $request->validate([
    ...
    private function mocklastNameGuess(?string $filename): string
    {
    ...
    private function findlastNameHeaderLocation(string $imagePath): ?array
    {
        try {
            ...
            for ($i = 0; $i < count($rows); $i++) {
                $current = $rows[$i];
                // Strip trailing punctuation (colon, period, space) that Tesseract commonly appends
                $currentText = rtrim(strtolower(trim((string) ($current['text'] ?? ''))), ':.; ');

                // Exact label match: "Last Name" with or without trailing colon/punctuation
                if ($currentText === 'last name') {
                    $best = $current;
                    break;
                }

                // OCR commonly splits into two tokens: "Last" + "Name" or "Last" + "Name:"
                if (isset($rows[$i + 1])) {
                    $next = $rows[$i + 1];
                    $nextText = rtrim(strtolower(trim((string) ($next['text'] ?? ''))), ':.; ');
                    $pair = trim($currentText . ' ' . $nextText);
                    if ($pair === 'last name') {
            ...
            \Log::warning('Error in findlastNameHeaderLocation: ' . $e->getMessage());
            return null;
        }
    }
    ...
    private function detectHandwrittenAreaAfterLabel(?string $imagePath, array $labelResult): ?array
    {
        try {
            ...
            $area = $this->detectHandwrittenAreaUsingWhitespaceEnd($imagePath, $labelResult);
            if (!$area) {
            ...
            return [
                ...
                'confidence' => null,
                'source'     => 'label-underline-whitespace-end',
                'underline'  => null,
                'anchors'    => $area['anchors'] ?? null,
            ];
        } catch (Throwable $e) {
            \Log::warning('Error detecting handwritten last-name area: ' . $e->getMessage());
            return null;
        }
    }

    /**
     * Step 3 for Last Name: use label + underline, then stop at whitespace after handwriting.
     * This does not depend on a trailing OCR label.
     */
    private function detectHandwrittenAreaUsingWhitespaceEnd(string $imagePath, array $labelResult): ?array
    {
        if (!is_file($imagePath)) {
            return null;
        }
        ...
        try {
            $labelLeft = (int) ($labelResult['x'] ?? 0);
            $labelTop = (int) ($labelResult['y'] ?? 0);
            $labelRight = (int) ($labelResult['x1'] ?? ($labelLeft + (int) ($labelResult['width'] ?? 0)));
            $labelBottom = (int) ($labelResult['y1'] ?? ($labelTop + (int) ($labelResult['height'] ?? 0)));
            $labelHeight = max(1, $labelBottom - $labelTop);

            if ($labelRight <= $labelLeft || $labelBottom <= $labelTop) {
                return null;
            }

            $underline = $this->detectUnderlineAfterLabel($imagePath, $labelResult);
            if (!$underline || !isset($underline['x'])) {
                return null;
            }

            [$img, $imgW, $imgH] = $this->loadImageForPixelScan($imagePath);
            if (!$img || $imgW < 2 || $imgH < 2) {
                return null;
            }

            $xStart = max(0, min($imgW - 1, (int) ($underline['x'] ?? ($labelRight + 5))));
            $scanXEnd = $imgW - 1;

            // Scan around where handwriting usually sits (roughly above the underline).
            $bandTop = max(0, min($imgH - 1, $labelTop - max(10, (int) round($labelHeight * 0.6))));
            $bandBottom = max($bandTop + 1, min($imgH - 1, $labelBottom + max(6, (int) round($labelHeight * 0.4))));

            $inkThreshold = 0.02;
            $blankThreshold = 0.005;
            $blankRunNeeded = max(20, min(70, (int) round($labelHeight * 1.5)));
            $darkThreshold = 175;
            ...
            for ($x = $xStart; $x <= $scanXEnd; $x++) {
                $coverage = $this->columnDarkCoverageRatio($img, $bandTop, $bandBottom, $x, $darkThreshold);
                ...
                if ($seenInk && $coverage <= $blankThreshold) {
                    $blankRun++;
                    if ($blankRun >= $blankRunNeeded) {
                        $xEnd = max($xStart + 10, $x - $blankRunNeeded + 2);
                        break;
                    }
                    continue;
                }
            ...
            if ($xEnd === null) {
                if ($seenInk) {
                    $xEnd = min($scanXEnd, $lastInkX + 12);
                } else {
                    $fallbackRight = (int) ($underline['x1'] ?? ($xStart + max(120, (int) round($labelHeight * 10))));
                    $xEnd = min($scanXEnd, max($xStart + 20, $fallbackRight));
                }
            }

            $boxLeft = $xStart;
            $boxRight = max($boxLeft + 20, min($scanXEnd, $xEnd));
            $boxHeight = min(125, max(95, $labelHeight * 1.27));
            $boxBottom = max(0, min($imgH - 1, $labelBottom + 3));
            ...
            return [
                ...
                'anchors' => [
                    ...
                    'whitespace_end_x' => (int) $boxRight,
                ],
            ];
        } catch (Throwable $e) {
            \Log::warning('Error detecting handwritten area by whitespace end: ' . $e->getMessage());
            return null;
        }
    }
    ...
    private function detectHandwrittenAreaBetweenLabels(string $imagePath, array $labelResult): ?array
    {
        ...
        try {
            ...
            // Step 3 bottom aligns to Step 1 marker bottom (Last Name label y1).
            ...
            \Log::warning('Error detecting area between Last Name and End of text: ' . $e->getMessage());
            return null;
        }
    }
    ...
    private function detectUnderlineAfterLabel(string $imagePath, array $labelResult): ?array
    {
        ...
        if (!$img || $imgWidth < 2 || $imgHeight < 2) {
            ...
            $x1 = min($x + max(120, (int) round($labelHeight * 10)), $imgWidth > 1 ? $imgWidth - 1 : $x + 120);
            $boxHeight = 15;
    ...
    private function columnDarkCoverageRatio($img, int $yStart, int $yEnd, int $x, int $darkThreshold): float
    {
        $pixels = max(1, $yEnd - $yStart + 1);
        $dark = 0;
    ...
    private function buildlastNameLocationJson(?array $labelResult, array $userInput, ?array $handwrittenResult): array
    {
        $location = [
            'label' => null,
            'handwritten' => [
