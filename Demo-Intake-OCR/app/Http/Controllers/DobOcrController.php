...
class DobOcrController extends Controller
{
    ...
    public function dobOcr()
    {
        try {
            ...
            $stateValues = $this->ocrStateAccessor->dob($globalRowData);
            ...
            return view('dob-ocr', [
                ...
                'humanValue' => null,
                'statusType' => 'error',
                'statusMessage' => 'Unable to load date of birth OCR state. ' . $exception->getMessage(),
            ]);
        }
    }
    ...
    public function findDobLabel()
    {
        try {
            ...
            // Step 1: Use Tesseract to find the "Date Of Birth:" label location
    ...
    /**
     * Step 2: find underline location after Date Of Birth label
     */
    public function findDobHandwritten()
    {
        try {
            $pendingData = $this->getSelectedPendingDataFromSession();
            $imagePath = $this->resolveOcrImagePath($pendingData);
            ...
            \Log::warning('Error in findDobHandwritten: ' . $e->getMessage());
            return response()->json([
                'ok' => false,
                'step' => $this->underlineFinder->stepContext('Date Of Birth OCR'),
                'message' => 'Error: ' . $e->getMessage(),
            ], 500);
        }
    }
    ...
    public function findDobArea()
    {
        try {
            ...
            $globalState = session('global_state');
            $locationData = is_array($globalState) && !empty($globalState['fp_dob_location'])
                ? json_decode($globalState['fp_dob_location'], true)
                : null;
            ...
            if (!$handwrittenResult) {
                return response()->json([
                    'ok' => false,
                    'message' => 'Could not find handwritten dob area after label.',
                ], 400);
            }
            ...
            $currentLocationData = [];
            if (!empty($globalState['fp_dob_location'])) {
                $currentLocationData = json_decode($globalState['fp_dob_location'], true) ?? [];
            }
            ...
            $state = array_merge($state, [
                ...
                'fp_dob_location' => json_encode($currentLocationData),
            ...
            return response()->json([
                'ok' => true,
                'message' => 'Handwritten dob area found successfully.',
                'handwritten' => $handwrittenResult,
            ]);
        } catch (Throwable $e) {
            \Log::warning('Error in findDobArea: ' . $e->getMessage());
            return response()->json([
                'ok' => false,
                'message' => 'Error: ' . $e->getMessage(),
            ], 500);
     ...
     */
    public function findDobOptions(Request $request)
    {
        try {
            $pendingData = $this->getSelectedPendingDataFromSession();
            $imagePath = $this->resolveOcrImagePath($pendingData);
            ...
            $state = array_merge($state, [
                'gs_current_image_path' => $pendingData['fp_image_path'] ?? null,
                'fp_dob_ocr' => $top['text'],
                'fp_dob_ocr_score' => $top['confidence'],
                'fp_dob_ocr_options' => json_encode($options),
                'updated_at' => now()->toDateTimeString(),
            ]);
            session(['global_state' => $state]);
            ...
            \Log::warning('Error in findDobOptions: ' . $e->getMessage());
            return response()->json([
                'ok' => false,
                'message' => 'Error: ' . $e->getMessage(),
            ], 500);
    ...
    public function runDobOcr(Request $request)
    {
        ...
        if ($selectedRecordId <= 0) {
            return redirect()->route('dob.index')->with([
                'statusType' => 'error',
        ...
        $guess = $this->mockDobGuess($filename);
        $score = $this->mockOcrScore($guess, $selectedRecordId);

        $globalState = session('global_state');
        $existingLocationData = [];
        if (is_array($globalState) && !empty($globalState['fp_dob_location'])) {
            $existingLocationData = json_decode($globalState['fp_dob_location'], true) ?: [];
        }
        ...
        $state = array_merge($state, [
            ...
            'fp_dob_location' => $locationJson,
            'fp_dob_ocr' => $guess,
            'fp_dob_ocr_score' => $score,
            'fp_dob_score' => $score,
            'updated_at' => now()->toDateTimeString(),
    ...
    public function confirmDobOcr(Request $request)
    {
        $validated = $request->validate([
        ...
        $state = array_merge($state, [
            'gs_current_image_path' => ($pendingData['fp_image_path'] ?? ($state['gs_current_image_path'] ?? null)),
            'fp_dob_human' => $validated['human_value'],
            'fp_dob_confirmed' => 1,
            'updated_at' => now()->toDateTimeString(),
    ...
    // -------------------------------------------------------------------------
    // Private helpers: Date Of Birth OCR specific
    // -------------------------------------------------------------------------
    ...
    private function mockDobGuess(?string $filename): string
    {
    ...
    private function findDobHeaderLocation(string $imagePath): ?array
    {
        try {
            ...
            if ($best === null) {
                // Final fallback: estimate underline span from Date Of Birth -> Last Name labels on the same row.
                $cmd = 'tesseract ' . escapeshellarg($imagePath) . ' stdout --psm 6 tsv 2>/dev/null';
                $tsv = shell_exec($cmd);
                if (is_string($tsv) && trim($tsv) !== '') {
                    $lines = preg_split('/\r\n|\r|\n/', trim($tsv)) ?: [];
            ...
            \Log::warning('Error in findDobHeaderLocation: ' . $e->getMessage());
            return null;
        }
    }
    ...
    private function detectHandwrittenAreaAfterLabel(?string $imagePath, array $labelResult): ?array
    {
        try {
            ...
            \Log::warning('Error detecting handwritten dob area: ' . $e->getMessage());
            return null;
        }
    }
    ...
    private function detectHandwrittenAreaBetweenLabels(string $imagePath, array $labelResult): ?array
    {
        ...
        try {
            ...
            // Step 3 bottom aligns to Step 1 marker bottom (Date Of Birth label y1).
            ...
            \Log::warning('Error detecting area between Date Of Birth and Last Name: ' . $e->getMessage());
            return null;
        }
    }
    ...
    private function detectUnderlineAfterLabel(string $imagePath, array $labelResult): ?array
    {
        ...
        // from right after Date Of Birth label to just before Last Name label.
        if ($anchorRight !== null) {
    ...
    private function buildDobLocationJson(?array $labelResult, array $userInput, ?array $handwrittenResult): array
    {
        $location = [
            'label' => null,
            'handwritten' => [
