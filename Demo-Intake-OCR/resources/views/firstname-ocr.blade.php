...
@section('content')
    <h1>First Name OCR</h1>
    <p class="lead">Run OCR using position (x,y), store location JSON in <code>fp_firstname_location</code>, then confirm the extracted text.</p>
    ...
    @else
        ...
        <div style="border:1px solid #d7dee6;border-radius:10px;background:#fff;padding:14px;margin-bottom:12px;">
            <h2 style="margin:0 0 10px;font-size:17px;">1) Find Location Of First Name Label</h2>
            <div style="display:flex;gap:14px;align-items:flex-start;">
                <div style="flex:0 0 auto;">
                    ...
                    <p style="margin:6px 0 0;color:#5f6b7a;font-size:12px;">
                        Saved as JSON into <code>global_state.fp_firstname_location.label</code>.
                    </p>
                </div>
                <div id="label-log"
        ...
        <div style="border:1px solid #d7dee6;border-radius:10px;background:#fff;padding:14px;margin-bottom:12px;">
            ...
            <div style="display:flex;gap:14px;align-items:flex-start;">
                <div style="flex:0 0 auto;">
                    ...
                    <p style="margin:6px 0 0;color:#5f6b7a;font-size:12px;">
                        Saves only the bounding area into <code>global_state.fp_firstname_location.handwritten</code>.
                    </p>
                </div>
                <div id="hand-log"
                     style="flex:1;min-width:220px;max-height:150px;overflow-y:auto;
        ...
        <div style="border:1px solid #d7dee6;border-radius:10px;background:#fff;padding:14px;margin-top:12px;">
            ...
            <form id="human-confirm-form" action="{{ route('firstname.confirm') }}" method="post" style="display:flex;gap:8px;flex-wrap:wrap;align-items:end;">
                @csrf
                <div style="min-width:260px;flex:1;">
                    <label style="display:block;font-size:12px;color:#475467;margin-bottom:4px;">Confirm First Name</label>
        ...
        <script>
            (function () {
                ...
                var labelUrl = @json(route('firstname.label'));
                var handwrittenUrl = @json(route('firstname.handwritten'));
                var areaUrl = @json(route('firstname.area'));
                var optionsUrl = @json(route('firstname.options'));
                ...
                var renderAllCanvases = function () {
                    ...
                    renderSliceOnCanvas('hand-slice-canvas', 'hand-slice-note', handSliceY, handLocation, '#0c8a43', false);
                ...
                if (labelBtn && labelLog) {
                    labelBtn.addEventListener('click', function () {
                        ...
                        labelLog.innerHTML = '';
                        appendLog(labelLog, 'Running Step 1: Find "First Name" label...');
